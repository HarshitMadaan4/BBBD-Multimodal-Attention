"""
train_exp3.py — Train Transformer & Mamba on Experiment 3
──────────────────────────────────────────────────────────
...
Results saved : results/  (metrics_exp3.json + confusion matrices + bar charts)
"""
...
def main():
    ...
    print(f"[Exp 3] EEG dim={eeg_dim}  Total trials={len(trials)}")
    ...
    out_json = os.path.join(RESULTS_DIR, "metrics_exp3.json")
    with open(out_json, 'w') as f:
        json.dump(all_metrics, f, indent=4)
    print(f"\nSaved metrics → {out_json}")

    save_summary_chart(all_metrics, WINDOW_SIZES, "Exp3", RESULTS_DIR)
